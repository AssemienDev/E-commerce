...
@WebServlet(name = "rechercheProd", value = "/recherche-prod")
public class RechercheProd extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String rech = request.getParameter("rechercher");

        // Établir la connexion à la base de données
        try (Connection connection = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD)) {
            String sqlProduits = "SELECT * FROM PRODUITS WHERE UPPER(NOMPRODUIT) LIKE UPPER(?)";
            try (PreparedStatement produitsStatement = connection.prepareStatement(sqlProduits)) {

                produitsStatement.setString(1, "%" + rech + "%".toUpperCase());
